...
def load_real_solomon_data(file_name: str, num_customers: int = 8) -> Dict[str, Any]:
...
class BenchmarkApp:
    def __init__(self, root):
        ...
        tk.Label(control_frame, text="File path:", font=("Helvetica", 10), bg="#f4f6f9").grid(row=0, column=0, padx=5)
        self.file_entry = tk.Entry(control_frame, font=("Helvetica", 10), width=12)
        self.file_entry.insert(0, "r101")
        self.file_entry.grid(row=0, column=1, padx=5)

        tk.Label(control_frame, text="Customers:", font=("Helvetica", 10), bg="#f4f6f9").grid(row=0, column=2, padx=5)
    ...
    def start_benchmark(self):
        ...
        t0 = time.time()
        res_bb = manual_branch_and_bound_vrptw(data, max_nodes=2000, node_selection="best_bound", time_limit_per_lp=15.0, use_initial_heuristic=False)
        ...
        t1 = time.time()
        res_bc = manual_branch_and_cut_vrptw(data, max_nodes=2000, node_selection="best_bound", time_limit_per_lp=15.0, use_initial_heuristic=False)
    ...
    def display_results(self, r_bb, t_bb, r_bc, t_bc):
        ...
        metrics_list = [
            ("Min cost", f"{r_bb['best_obj']:.4f}" if r_bb['best_obj'] != math.inf else "No Sol", f"{r_bc['best_obj']:.4f}" if r_bc['best_obj'] != math.inf else "No Sol"),
            ("Time", f"{t_bb:.4f}", f"{t_bc:.4f}"),
            ("Nodes", str(r_bb["stats"]["nodes_solved"]), str(r_bc["stats"]["nodes_solved"])),
            ...
            ("optimized", str(r_bb["optimal_proved"]), str(r_bc["optimal_proved"]))
        ]
